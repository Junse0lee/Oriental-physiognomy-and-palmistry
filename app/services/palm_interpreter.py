...
class PalmInterpreter:
    def __init__(self):
        ...
        self.texts = {
            ...
            'head_shape_book': {
                'straight': ["📏 <b>[현실적인 분석가]</b><br>당신은 감정보다는 사실(Fact)을 중요하게 생각합니다. 겉으로는 차가워 보일 수 있지만, 사실은 누구보다 합리적으로 상황을 해결하여 주변 사람들을 보호하려는 책임감이 강합니다.", 
                ...
                'gentle': ["🌊 <b>[유연한 사고의 소유자]</b><br>논리와 직관 사이에서 줄타기를 아주 잘하는군요. 현실적인 감각을 유지하면서도 타인의 감정을 읽어내는 센스까지 갖추고 있어, 어떤 상황에서도 유연하게 대처합니다.", 
                ...
                'curved': ["🎭 <b>[풍부한 상상력의 아티스트]</b><br>남들이 보지 못하는 것을 보는 눈을 가졌습니다. 논리적인 설명보다는 순간적인 영감이나 직관이 더 잘 맞을 때가 많으며, 틀에 박힌 것을 거부하는 자유로운 영혼입니다.", 
            ...
            'head_destination_book': {
                ...
                'moon_upper': "👑 <b>[사람을 이끄는 리더]</b><br>현실적 문제 해결과 비전을 동시에 갖췄습니다. 상식적이면서도 책임감이 강해 조직을 이끄는 리더나 전문직으로서 존경받을 운명입니다.",
            ...
            # --- [감정선] ---
            'heart_start_book': {
                'standard': ["⚖️ <b>[이성과 감정의 황금비율]</b><br>어떤 상황에서도 평정심을 유지할 줄 아는군요. 너무 차갑지도 뜨겁지도 않은 온도로 사람을 대하기에 실수가 없고 신뢰받습니다.", 
                             "⚖️ <b>[믿음직한 파트너]</b><br>당신은 감정에 휘둘리지 않고 중심을 잡을 줄 압니다. 연애에서도 뜨거운 불꽃보다는 따뜻한 난로처럼 은은하고 오래가는 관계를 선호하는군요." ],
                'high': ["🔥 <b>[브레이크 없는 열정]</b><br>사랑에 빠지면 물불 가리지 않는 열정파입니다! 감정 표현이 화끈하지만, 때로는 질투심이나 소유욕이 불타오르기도 합니다.", 
                ...
                'low': ["❄️ <b>[침착한 포커페이스]</b><br>겉으로 감정을 잘 드러내지 않아 '속을 모르겠다'는 말을 듣지만, 사실 누구보다 깊고 진중한 마음을 가졌습니다. 의리파입니다.", 
            ...
            'heart_shape_book': {
                'straight': ["📏 <b>[돌직구 직진남녀]</b><br>밀당? 그런 건 딱 질색이시죠. 좋으면 좋다, 싫으면 싫다 확실하게 표현하는 솔직 담백한 성격입니다. 뒤끝이 없어 쿨한 매력이 돋보입니다.", 
                ...
                'curved': ["🌊 <b>[따뜻한 감성 힐러]</b><br>타인의 감정을 내 것처럼 느끼는 공감 능력을 가졌습니다. 상대방의 기분을 먼저 살피는 따뜻한 마음씨 덕분에 주변에 사람이 끊이지 않습니다.", 
                ...
                'hybrid': ["🎭 <b>[알다가도 모를 반전 매력]</b><br>냉정과 열정 사이를 오가는 양면성이 있습니다. 이 '종잡을 수 없는 매력'이 이성에게는 치명적인 호기심을 자극합니다. 밀당의 고수시군요!", 
            ...
            'heart_end_book': {
                ...
                'middle_down': "😎 <b>[쿨내 진동 자유인]</b><br>연애는 인생의 일부일 뿐입니다. 구속받는 것을 싫어하고 프라이버시를 중요하게 생각합니다. 쿨한 매력이 인기의 비결입니다.",
            ...
            # --- [운명선] ---
            'fate_path': {
                 'support': ["🤝 <b>[최고의 참모: 2인자의 미학](옅은 운명선)</b><br>당신의 운명선은 은은하게 흐르는 시냇물 같습니다. 이는 리더를 전면에서 이끌기보다, 보이지 않는 곳에서 판을 짜고 조직을 완성하는 '킹메이커'의 기질을 뜻합니다.",
    ...
    def interpret(self, features, mounts, hand_metrics):
        ...
        if 'life' in features:
            f = features['life']
            msg = [self._pick(self.texts['life_start'][['glue','tight','normal_tight','normal','normal_loose','loose','free','free'][self.get_step(features.get('head_life_gap',0), self.TH['gap'])]])]
            msg.append("<br>" + self.texts['life_length_book'][self.analyze_life_length(f, hand_metrics['height'], mounts['地'][1])])
            msg.append("<br>" + self.texts['life_curve_book'][self.analyze_life_curve(f, mounts)])
            msg.append("<br>" + self.texts['life_direction_book'][self.analyze_life_dir(f, mounts)])
            cards.append({"title": "🌿 생명선", "color": "#78ffe6", "text": "".join(msg)})

        # 2. 두뇌선
        if 'head' in features:
            f = features['head']
            msg = [self.texts['head_length_book'][self.analyze_head_len(f, mounts)]]
            msg.append("<br>" + self._pick(self.texts['head_shape_book'][self.analyze_head_shape(f)]))
            msg.append("<br>" + self.texts['head_destination_book'][self.analyze_head_dest(f, mounts, hand_metrics['height'])])
            cards.append({"title": "🧠 두뇌선", "color": "#ffe696", "text": "".join(msg)})

        # 3. 감정선
        if 'heart' in features:
            f = features['heart']
            msg = [self._pick(self.texts['heart_start_book'][self.analyze_heart_start_book(f, hand_metrics['height'], mounts['수성'][1])])]
            msg.append("<br>" + self._pick(self.texts['heart_shape_book'][self.analyze_heart_shape_book(f)]))
            msg.append("<br>" + self.texts['heart_end_book'][self.analyze_heart_end_book(f, mounts)])
            cards.append({"title": "❤️ 감정선", "color": "#ff7878", "text": "".join(msg)})

        # 4. 운명선 (M자 + 8가지 경로 완전 복구)
        if 'fate' in features:
            f, fate_msgs = features['fate'], []
            is_m = False
            if 'head' in features and 'heart' in features:
                fate_pts = f['points']
                if np.max(fate_pts[:, 1]) > np.mean(features['head']['points'][:, 1]) and np.min(fate_pts[:, 1]) <= (np.mean(features['heart']['points'][:, 1]) + (hand_metrics['height']*0.05)):
                    is_m = True
            if is_m: fate_msgs.append(self.texts['special_signs']['m_sign'])
            
            if f['conf'] < 0.4 and not is_m:
                fate_msgs.extend(random.sample(self.texts['fate_path']['support'], 2))
            else:
                fate_msgs.append(self._pick(self.texts['fate_start_book'][self.analyze_fate_start_book(f, mounts, features)]))
                fate_msgs.append(self._pick(self.texts['fate_end_book'][self.analyze_fate_end_book(f, mounts, features)]))
            
            if f['conf'] > 0.35 and not is_m: fate_msgs.append(self.texts['special_signs']['rich_triangle'])
            fate_msgs.append("<br>" + random.choice(self.texts['fate_advice']))
            cards.append({"title": "🌟 운명선", "color": "#8c96ff", "text": "<br><br>".join(fate_msgs)})
        else:
            cards.append({"title": "🌟 운명선", "color": "#8c96ff", "text": self.texts['special_signs']['no_fate']})

        # 5. Forbidden
        spicy = []
        if 'life' in features: spicy.append(self._pick(self.texts['libido']['level_5' if self.analyze_life_curve(features['life'], mounts)=='big' else 'level_3']))
        if 'heart' in features: spicy.append(self._pick(self.texts['seduction']['hot_lover' if features['heart']['curv']>1.12 else 'shy_fox']))
        if 'head' in features: spicy.append(self._pick(self.texts['fantasy']['deep' if abs(features['head']['slope'])>0.45 else 'real']))
        
        full_spicy = "<br><br>".join(spicy) + f"<br><br><div style='color:#ff4d4d; font-weight:bold;'>{self._pick(self.texts['outro'])}</div>"
        cards.append({"title": self.texts['spicy_title'], "color": "#e74c3c", "text": full_spicy})

        # --- HTML 조립 ---
        full_html = '<div class="palm-scroll-area">'
        for card in cards:
            full_html += f'<div class="report-card" style="border-top: 8px solid {card["color"]};"><h3 class="card-title">{card["title"]}</h3><div class="card-content">{card["text"]}</div></div>'
        full_html += '</div>'
        return full_html